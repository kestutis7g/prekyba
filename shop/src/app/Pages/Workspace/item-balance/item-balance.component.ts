...
export class ItemBalanceComponent implements OnInit {
    ...
    this.itemBalanceService.getItemBalanceListByItemId(this.itemId).subscribe({
      next: (data) => {
        ...
        if (this.range.value.start && this.range.value.end) {
          let start = new Date(
            this.range.value.start.format('YYYY-MM-DD').toString()
          );
          let end = new Date(
            this.range.value.end.format('YYYY-MM-DD').toString()
          );
          ...
          this.itemBalanceList.forEach((balance) => {
            ...
            if (
              balanceDate.getTime() >= start.getTime() &&
              balanceDate.getTime() <= end.getTime()
            ) {
  ...
  getPrediction3() {
    ...
    previousBalance =
      this.itemBalanceList[this.itemBalanceList.length - 1].amount;
  ...
  salesData: ChartData<'line'> = {
    labels: [],
    datasets: [{ label: this.label, data: [], tension: 0.5 }],
  };
